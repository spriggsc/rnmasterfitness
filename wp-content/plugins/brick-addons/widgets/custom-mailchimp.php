<?php
	...
	add_action('widgets_init', 'Brick_register_mailchimp_widget');
	function Brick_register_mailchimp_widget(){
		register_widget( 'Brick_WP_Widget_Mailchimp' );
	}

	class  Brick_WP_Widget_Mailchimp extends WP_Widget {

		public function __construct() {
			parent::__construct(
				'mailchimp',
				esc_html__('* Mailchimp', 'brick'),
				array( 'description' => esc_html__('MailChimp Signup Form', 'brick'))
			);
		}
		...
		public function widget( $args, $instance ) {
			...
			$output .='<div id="mc_embed_signup">';
				$output .= '<div class="mailchimp-subtitle form">'.$subtitle.'</div>';
				$output .='<form action="'.$action.'" method="post" id="mc-embedded-subscribe-form" name="mc-embedded-subscribe-form" class="validate" target="_blank" novalidate>';
					...
					$output .='<input type="text" value="" name="EMAIL" class="email mailing-list-email-input" id="mce-EMAIL" data-placeholder="Your Email" required>';
				    $output .='<input type="text" name="'.$name.'" tabindex="-1" value="" class="hidden">';
				    $output .='<input class="btn mailing-list-submit-btn" type="submit" value="'.esc_html__('Subscribe now!', 'brick').'" name="subscribe" id="mc-embedded-subscribe">';
				$output .='</form>';
	 	...
	 	public function form( $instance ) {
			...
			$defaults = array(
	 			'title'       => esc_html__('Subscribe', 'brick'),
	 			'action'      => '',
	 			'name'        => '',
	 			'description' => '',
	 			'subtitle'    => ''
			...
			<p>
				<label for="<?php echo $this->get_field_id( 'subtitle' ); ?>"><?php echo esc_html__( 'Subtitle:', 'brick' ); ?></label> 
			...
			<p>
				<label for="<?php echo $this->get_field_id( 'action' ); ?>"><?php echo esc_html__( 'Action:', 'brick' ); ?></label> 
			...
			<p>
				<label for="<?php echo $this->get_field_id( 'name' ); ?>"><?php echo esc_html__( 'Name:', 'brick' ); ?></label> 
			...
			<p>
				<label for="<?php echo $this->get_field_id( 'description' ); ?>"><?php echo esc_html__( 'Description:', 'brick' ); ?></label> 
